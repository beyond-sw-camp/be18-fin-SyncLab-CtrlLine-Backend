...
class ItemTest {
    ...
    @Test
    @DisplayName("Item 수정 시 공백으로 전달된 필드는 기존 값이 유지된다.")
    void updateItem_blankFields_ignored() {
        // given
        Items item = Items.builder()
                .itemCode("ITEM-007")
                .itemName("3상 차단기")
                .itemSpecification("15A / 220V")
                .itemUnit("EA")
                .itemStatus(ItemStatus.FINISHED_PRODUCT)
                .isActive(true)
                .build();
        ...
        // then
        assertThat(item.getItemCode()).isEqualTo("ITEM-007"); // 유지
        assertThat(item.getItemName()).isEqualTo("3상 차단기"); // 유지
        assertThat(item.getItemSpecification()).isEqualTo("15A / 220V"); // 유지
        assertThat(item.getItemUnit()).isEqualTo("EA"); // 유지
        assertThat(item.getItemStatus()).isEqualTo(ItemStatus.FINISHED_PRODUCT); // 유지
    }
}
